...
def run_delay_experiment(delay_seconds: float) -> DelayResult:
    ...
    state_buffer: deque[tuple[float, float]] = deque(
        [
            (
    ...
    for step_index in range(total_steps + 1):
        ...
        if step_index % physics_steps_per_control_step == 0:
            ...
            # 控制器读取缓冲区最旧的数据。
            observed_position, observed_velocity = (
                state_buffer[0]
            )

            raw_torque = pd_control(
                current_position=observed_position,
                current_velocity=observed_velocity,
...
def plot_position_results(
    ...
    """Plot position responses for all observation delays."""
    ...
    plt.title("Effect of Observation Delay on PD Control")
    plt.grid()
    plt.legend()
    plt.tight_layout()
...
def plot_torque_results(
    ...
    """Plot applied torque for all observation delays."""
    ...
    plt.title("Applied Torque with Observation Delay")
    plt.grid()
    plt.legend()
    plt.tight_layout()
...
def main() -> None:
    """Run the observation-delay sweep."""
